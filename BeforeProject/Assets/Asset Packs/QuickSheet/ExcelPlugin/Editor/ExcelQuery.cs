...
namespace UnityQuickSheet
{
    ...
    public class ExcelQuery
    {
        ...
        public List<T> Deserialize<T>(int start = 1)
        {
            ...
            Debug.Log($"ExcelQuery Deserialize. Path: {filepath}. Sheet: {sheet.SheetName}");
            //foreach (var property in p)
            //{
            //    Debug.Log($"있는 것들: {property.Name}");
            //}

            int current = 0;
            string error = "";
            string[] originTitles = GetTitle(0, ref error);
            ...
            foreach (IRow row in sheet)
            {
                ...
                {
                    // title이 없는 것이 나오면 i를 추가 시켜서 undefined를 무시
                    // 순서가 중요하므로 무작위 순서여서는 안 됨
                    if (Array.Find(p, (property) => property.Name == originTitles[i]) == null)
                    {
                        //Debug.Log($"{originTitles[i]}는 없습니다.");
                        continue;
                    }

                    // custom: 빈공간은 빈공간으로서 받음
                    ICell cell = row.GetCell(i, MissingCellPolicy.CREATE_NULL_AS_BLANK);

                    if (cell == null)  // skip empty cell
                        continue;

                    var property = p[propertyIndex++];
                    if (property.CanWrite)
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            Debug.LogError(string.Format("Excel File {0} at {1}, Deserialize Exception: {2} at {3}", this.filepath, sheet.SheetName, e.Message, pos));
                        }
                    }
                }
        ...
        protected object ConvertFrom(ICell cell, Type t)
        {
            ...
            else if (t == typeof(string) || t.IsArray)
            {
                ...
                {
                    if (cell.CachedFormulaResultType == NPOI.SS.UserModel.CellType.Numeric) value = cell.NumericCellValue;
