...
namespace UnityQuickSheet
{
    internal class ScriptGenerator
    {
        ...
        private string DataRowStartIndex
        {
            get
            {
                return m_ScriptPrescription.dataRowStartIndex.ToString();
            }
        }

        private string PrefixStrLen
        {
            get
            {
                return m_ScriptPrescription.prefixStrLen.ToString();
            }
        }

        private string Prefix
        {
            get => m_ScriptPrescription.prefix;
        }
        ...
        public override string ToString()
        {
            ...
            // custom: 데이터 시작점 추가
            m_Text = m_Text.Replace("$DataRowStartIndex", DataRowStartIndex);
            m_Text = m_Text.Replace("$PrefixStrLen", PrefixStrLen);
            m_Text = m_Text.Replace("$Prefix", Prefix);
        ...
        protected virtual string GetFieldNameForField(MemberFieldData field)
        {
            ...
            //custom 첫문자만 소문자 변환
            return field.Name.First().ToString().ToLower() + field.Name.Substring(1);
        }
        ...
        protected virtual string GetPropertyNameForField(MemberFieldData field)
        {
            ...
            return field.Name.First().ToString().ToUpper() + field.Name.Substring(1);
