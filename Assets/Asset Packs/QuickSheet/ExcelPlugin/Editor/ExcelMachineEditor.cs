    ...
    public class ExcelMachineEditor : BaseMachineEditor
    {
        ...
        protected override void Import(bool reimport = false)
        {
            ...
            else
            {
                ...
                {
                    var types = machine.isDataTypeIndex ? new ExcelQuery(path, sheet).GetTitle(machine.dataTypeIndex, ref error) : null;
                    machine.ColumnHeaderList = new List<ColumnHeader>();
                    for (var i = 0; i < titleList.Count; i++)
                    {
                        // Debug.Log($"title {titleList[i]}, type: {types[i]}");
                        machine.ColumnHeaderList.Add(ParseColumnHeader(titleList[i], i, machine.isDataTypeIndex ? types[i] : null));
                    }
                }
                else
                {
        ...
        protected override void CreateAssetCreationScript(BaseMachine m, ScriptPrescription sp)
        {
            ...
            //
            string xlName = $"{machine.SpreadSheetName}".Split('.')[0];
            string folderPath = $"Assets/Address/Data/{xlName}";

            if (Directory.Exists(folderPath) == false)
                Directory.CreateDirectory(folderPath);

            string path = $"{folderPath}/{machine.WorkSheetName}.asset";
            //
            sp.assetFilepath = path.Replace('\\', '/');
            sp.assetPostprocessorClass = machine.prefixString + machine.WorkSheetName + "AssetPostprocessor";
